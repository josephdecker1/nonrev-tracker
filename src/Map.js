...
const MapIcon = () => <div css={ css`${mapMarkerStyle}` }><MdAirplanemodeActive css={ css`${iconStyle}` } /></div>;
...
const Map = (props) => {
  ...
  const createMapIcons = () => {
    let components = props.flightData.flatMap(
      (flight) => [
        (<MapIcon key={ `${flight.PNR}-${Math.floor(Math.random() * 10000) + 1}` }
  ...
  const animateFlight = (flight) => {
    ...
    var interval = setInterval(function () {
      ...
      if (step > numSteps) {
        ...
        var are_we_there_yet = maps.geometry.spherical.interpolate(new maps.LatLng(flight_lines[0]), new maps.LatLng(flight_lines[1]), step / numSteps);
